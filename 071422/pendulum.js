...
//     //     i%2===0 ? result.unshift(input[i]) : result.push(input[i]);
//     // }
//     input.map((el, index) => index%2 === 0 ? result.unshift(el) : result.push(el));
//     return result;
// }
...
function pendulum(values) {
    let output = [];
    values.sort((a,b)=>a-b);
    let left = values.filter((e,i)=>i%2 === 0);
    let right = values.filter((e,i)=>i%2 > 0);
    output = [...left.reverse(), ...right];
    return output;
}
